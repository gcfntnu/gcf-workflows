...
def read_doublet_file(fn, verbose=False):
    """
    Reads a doublet file, extracts droplet type and donor information.
    ...
    Returns:
        tuple: (doublet_call, donor_id) DataFrames.
    """
    doublet_call, donor_id = None, None
    name = os.path.basename(os.path.dirname(fn))
    
    # Try reading as tab-delimited file; fall back to comma-separated if necessary
    try:
        df = pd.read_table(fn, index_col=0, sep="\t", dtype=str)
        if df.shape[1] == 0:
            raise ValueError("Empty DataFrame with tab separator")
    except Exception as e:
        df = pd.read_table(fn, index_col=0, sep=",", dtype=str)
    
    # Select columns that contain the value "singlet"
    # Using df.eq and any() to find columns with 'singlet'
    droplet_cols = df.eq("singlet").any()
    doublet_call = df.loc[:, droplet_cols]
    ...
    # Process donor information if present
    if "donor_id" in df.columns:
        # If regex-based splitting is intended, use re.sub to remove undesired trailing parts
        df["donor_id"] = df["donor_id"].apply(lambda i: re.sub(r'\.*$', '', i))
        if "best_singlet" in df.columns:
            df["best_singlet"] = df["best_singlet"].apply(lambda i: re.sub(r'\.*$', '', i))
            donor_id = df[["donor_id", "best_singlet"]].copy()
            donor_id.columns = [f"{name}_donor_id", f"{name}_best_singlet"]
        else:
            donor_id = pd.DataFrame(df["donor_id"]).copy()
...
def upset_plot_doublets(df, output_dir, prefix="", stacked_bars=None):
    ...
    if isinstance(stacked_bars, str):
        upset = upsetplot.UpSet(dat, min_subset_size=100, sort_by="cardinality", intersection_plot_elements=0,
                                show_counts=True, show_percentages=False, orientation="vertical", element_size=100)
