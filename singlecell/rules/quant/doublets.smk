...
if SAMPLE_MULTIPLEXING:
    if os.path.exists(workflow.source_path("multiplex.smk")):
        include:
            'multiplex.smk'
    else:
...
rule dbl_doubletdetection:
    ...
    params:
        script = workflow.source_path('scripts/run_doubletdetection.py')
    threads:
        8
    shell:
...
rule dbl_scdblfinder:
    ...
    params:
        script = workflow.source_path('scripts/scdblfinder.R')
    shell:
        '{params.script} '
        '-i {input.counts} '
        '-o {output}'
...
rule dbl_scds:
    ...
    params:
        script = workflow.source_path('scripts/scds.R')
    shell:
        '{params.script} '
        '-i {input.counts} '
        '-o {output} '
...
rule scrublet:
    ...
    params:
        script = workflow.source_path('scripts/run_scrublet.py')
    threads:
        8
    shell:
        'python {params.script} '
...
rule dbl_solo_summary:
    ...
    params:
        script = workflow.source_path('scripts/solo_summary.py')
    container:
        'docker://gcfntnu/solo-sc:1.2'
    shell:
        'python {params.script} '
...
rule dbl_socube_summary:
    ...
    params:
        script = workflow.source_path('scripts/socube_summary.py')
    container:
...
rule dbl_majority_vote_per_sample:
    input:
        get_doublet_output()
    params:
        script = workflow.source_path("scripts/combine_doublets.py"),
        args = '--plot-figure '
    output:
...
rule dbl_aggr:
    ...
    params:
        script = workflow.source_path("scripts/combine_demultiplex.py")
    container:
        'docker://' + config['docker']['default']
    shell:
        'python {params.script} '
