#!/usr/bin/env python3
"""Comparison harness for refined automatic-QC distribution diagnostics.

This script deliberately reuses the MAD threshold implementation in
``qc_mad.py`` unchanged. It only replaces the advisory modality diagnostic and
diagnostic plotting so the refined behavior can be validated against real
projects before being folded into the production implementation.
...
Neither diagnostic changes the MAD-derived QC thresholds.
"""
...
def modality_diagnostic(x_t: np.ndarray) -> RefinedModalityResult:
    x = np.asarray(x_t, dtype=np.float64)
    x = x[np.isfinite(x)]
    if x.size < 10:
        return RefinedModalityResult(failed=True)
...
def plot_metric_distribution(
    *,
    ...
    modality: RefinedModalityResult,
    status: str,
    flags: Sequence[str],
    n_total: int,
    n_fit: int,
...
# Replace diagnostics only; the MAD estimator and all QC decisions stay in qc_mad.py.
